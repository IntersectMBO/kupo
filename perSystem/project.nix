      ...
      modules = [
        {
          ...
          # Substitute the TH splice with a literal so nix-built binaries
          # report the git sha without needing git in the sandbox.
          packages.kupo.components.library.preBuild = ''
            substituteInPlace src/Kupo/Version.hs \
              --replace-fail '$(gitRevisionTH)' '"${
              let
                rev = self.rev or self.dirtyRev or "";
              in
                if rev == ""
                then ""
                else "+" + builtins.substring 0 6 rev
            }"' \
              --replace-fail '    ( gitRevisionTH' '    ('
          '';
